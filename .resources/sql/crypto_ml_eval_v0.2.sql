...
CREATE TABLE IF NOT EXISTS `randf_evaluation` (
  ...
  `randf_hyperparams_uuid` varchar(50) NOT NULL,
  ...
  PRIMARY KEY (`uuid`),
  UNIQUE KEY `unique_params` (`candle_size`,`metric`,`n`,`margin`,`alpha`),
  KEY `randf_hyperparams_fk` (`randf_hyperparams_uuid`),
  KEY `randf_precision_fk` (`precision_scores_uuid`),
  KEY `randf_recall_fk` (`recall_scores_uuid`),
  KEY `randf_f1_fk` (`f1_scores_uuid`),
  CONSTRAINT `randf_f1_fk` FOREIGN KEY (`f1_scores_uuid`) REFERENCES `f1_scores` (`uuid`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `randf_hyperparams_fk` FOREIGN KEY (`randf_hyperparams_uuid`) REFERENCES `randf_hyperparams` (`uuid`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `randf_precision_fk` FOREIGN KEY (`precision_scores_uuid`) REFERENCES `precision_scores` (`uuid`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `randf_recall_fk` FOREIGN KEY (`recall_scores_uuid`) REFERENCES `recall_scores` (`uuid`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC;
...
CREATE TABLE IF NOT EXISTS `svm_evaluation` (
  ...
  `svm_hyperparams_uuid` varchar(50) NOT NULL,
  `precision_scores_uuid` varchar(50) DEFAULT NULL,
  `recall_scores_uuid` varchar(50) DEFAULT NULL,
  `f1_scores_uuid` varchar(50) DEFAULT NULL,
  PRIMARY KEY (`uuid`),
  UNIQUE KEY `unique_params` (`metric`,`n`,`margin`,`alpha`,`candle_size`),
  ...
  KEY `svm_hyperparams_fk` (`svm_hyperparams_uuid`),
  CONSTRAINT `svm_f1_fk` FOREIGN KEY (`f1_scores_uuid`) REFERENCES `f1_scores` (`uuid`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `svm_hyperparams_fk` FOREIGN KEY (`svm_hyperparams_uuid`) REFERENCES `svm_hyperparams` (`uuid`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `svm_precision_fk` FOREIGN KEY (`precision_scores_uuid`) REFERENCES `precision_scores` (`uuid`) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `svm_recall_fk` FOREIGN KEY (`recall_scores_uuid`) REFERENCES `recall_scores` (`uuid`) ON DELETE SET NULL ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
